...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ###         src_len = maximum source sentence length, b = batch size, e = embedding size. Note
        ###         that there is no initial hidden state or cell for the decoder.
        tensor_x = self.model_embeddings.source(source_padded)
        tensor_x = pack_padded_sequence(tensor_x,lengths=source_lengths)
        ...
        ###         - Note that the shape of the tensor returned by the encoder is (src_len b, h*2) and we want to
        ###           return a tensor of shape (b, src_len, h*2) as `enc_hiddens`.
        enc_hiddens, (last_hidden, last_cell) = self.encoder(tensor_x)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###     2. Construct tensor `Y` of target sentences with shape (tgt_len, b, e) using the target model embeddings.
        ###         where tgt_len = maximum target sentence length, b = batch size, e = embedding size.
        tensor_y = self.model_embeddings.target(target_padded)
        ...
        ###             - Append o_t to combined_outputs
        ###             - Update o_prev to the new o_t.
        for tensor_y_time in torch.split(tensor_y,1,dim=0):
            squeezed_y = torch.squeeze(tensor_y_time)
